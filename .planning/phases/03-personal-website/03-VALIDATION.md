...
- [ ] Verify `OpenSpatialDelay v1.0.component` AU plugin installed in `~/Library/Audio/Plug-Ins/Components/`; if missing, run `bash scripts/build_version.sh 768c248 v1.0.0 O100` in plugin repo before Wave 2 screenshot capture
...
| Plugin v1.0.0 UI capture fidelity | WEB-01 (D-10) | Requires live DAW session (REAPER) with correct plugin version loaded | Launch REAPER, open blank project, insert `OpenSpatialDelay v1.0` (AU or VST3), confirm plugin window title reads "OpenSpatialDelay v1.0", capture 11 screenshots per UI-SPEC asset list using `Cmd+Shift+4+Space`, save with naming convention `screenshot_<name>.png` to `../andrewrahman-com/public/assets/` |
