    ...
    OpenSpatialDelayProcessor::createParameterLayout()
{
    ...
    params.push_back (std::make_unique<juce::AudioParameterChoice> (
        juce::ParameterID ("algorithm", 4), "Algorithm",
        juce::StringArray { "Ambisonics (HOA)", "DBAP", "KNN", "VBAP", "VBIP" }, 0));
    ...
    params.push_back (std::make_unique<juce::AudioParameterBool> (
        juce::ParameterID ("airAbsorption", 4), "Air Absorption", true));
      ...
      apvts (*this, nullptr, "Parameters", createParameterLayout())
{
    ...
    // 5 algorithms (alphabetical): Ambisonics (0), DBAP (1), KNN (2), VBAP (3), VBIP (4)
    algorithms[0] = &algAmbisonics;
    algorithms[1] = &algDBAP;
    algorithms[2] = &algKNN;
    algorithms[3] = &algVBAP;
    algorithms[4] = &algVBIP;
}
                                               ...
                                               juce::MidiBuffer& /*midi*/)
{
    ...
    if (! isBinaural && ! algo->supportsSurround())
        algo = algorithms[3];  // Fall back to VBAP (index 3 in alphabetical order)
    ...
    // --- v0.4: Per-block Doppler velocity computation + air absorption filter update ---
    {
        ...
        for (int t = 0; t < MAX_OBJECTS; ++t)
        {
            ...
            auto prefix = "object" + juce::String (t + 1) + "_";
            float objDopplerAmount = apvts.getRawParameterValue (prefix + "dopplerAmount")->load();
            ...
            {
                // Bypass: set cutoff to 20kHz (transparent)
                *airAbsorptionFilter[t].coefficients =
                    *juce::dsp::IIR::Coefficients<float>::makeLowPass (currentSampleRate, 20000.0f);
            }
        }
    ...
    if (isBinaural && useHRTF)
    {
        ...
        for (int s = 0; s < numSamples; ++s)
        {
            ...
            for (int t = 0; t < MAX_OBJECTS; ++t)
            {
                ...
                float objPitch = static_cast<float> (t + 1) * pitchSemitones + dopplerSemitones[t];

                float objMono = readPitchShifted (objDelaySamples, objPitch, t);
                objMono = airAbsorptionFilter[t].processSample (objMono);
