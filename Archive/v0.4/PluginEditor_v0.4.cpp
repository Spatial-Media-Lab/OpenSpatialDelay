    ...
    : AudioProcessorEditor (&p), processorRef (p)
{
    ...
    // --- Title ---------------------------------------------------------------
    titleLabel.setText ("OpenSpatialDelay v0.4", juce::dontSendNotification);
    ...
    setupCombo (algorithmBox,   algorithmLabel,   "ALGORITHM",
                "algorithm", { "Ambisonics", "DBAP", "KNN", "VBAP", "VBIP" }, algorithmAttach);
...
void OpenSpatialDelayEditor::timerCallback()
{
    ...
    if (isBinaural)
    {
        // Binaural: hide algorithm dropdown entirely (rendering is always Direct Binaural)
