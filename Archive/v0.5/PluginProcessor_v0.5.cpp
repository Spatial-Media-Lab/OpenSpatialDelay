    ...
    OpenSpatialDelayProcessor::createParameterLayout()
{
    ...
    for (int i = 0; i < MAX_OBJECTS; ++i)
    {
        ...
        // v0.5: Trajectory parameter stubs (engine deferred to v0.6)
        params.push_back (std::make_unique<juce::AudioParameterChoice> (
            juce::ParameterID ("object" + juce::String (i + 1) + "_trajectoryShape", 5),
            name ("Trajectory Shape"),
            juce::StringArray { "None", "Spiral", "Orbit", "Bounce", "Figure-8", "Random" }, 0));

        params.push_back (std::make_unique<juce::AudioParameterFloat> (
            juce::ParameterID ("object" + juce::String (i + 1) + "_trajectorySpeed", 5),
...
void OpenSpatialDelayProcessor::getStateInformation (juce::MemoryBlock& destData)
{
    ...
    state.setProperty ("pluginStateVersion", 8, nullptr);  // v0.5 state format marker (8 = Binaural-first ordering)
