...
void SpatialMapComponent::paint (juce::Graphics& g)
{
    ...
    g.drawText ("F",  juce::roundToInt (cx - 10), juce::roundToInt (cy - radius - 20), 20, 16, juce::Justification::centred);
    g.drawText ("B",  juce::roundToInt (cx - 10), juce::roundToInt (cy + radius + 4),  20, 16, juce::Justification::centred);
    g.drawText ("L",  juce::roundToInt (cx - radius - 22), juce::roundToInt (cy - 8),  20, 16, juce::Justification::centred);
    g.drawText ("R",  juce::roundToInt (cx + radius + 4),  juce::roundToInt (cy - 8),  20, 16, juce::Justification::centred);
    ...
    : AudioProcessorEditor (&p), processorRef (p)
{
    ...
    oscPortLabel.setColour (juce::Label::backgroundColourId, juce::Colour (0xff151525));
    oscPortLabel.setColour (juce::Label::outlineColourId, juce::Colour (0xff334155));  // matches ComboBox outline
...
void OpenSpatialDelayEditor::paint (juce::Graphics& g)
{
    ...
    // --- v0.6: OSC toggle button (right panel, custom-painted) ---
    {
        ...
        if (bounds.getWidth() > 0)
        {
            ...
            // "Port:" label between toggle and port field
            g.setColour (Colours_OSD::textSecondary);
            g.setFont (juce::FontOptions (10.0f));
            g.drawText ("Port:", (int)(bounds.getRight() + 8), (int)bounds.getY(),
                        34, (int)bounds.getHeight(), juce::Justification::centredLeft);
        }
    }
...
void OpenSpatialDelayEditor::resized()
{
    ...
    // v0.6: Preset browser controls (consistent height with other header dropdowns)
    rx -= 44;
    presetSaveButton.setBounds (rx, boxY, 44, boxH);
    rx -= 24;
    presetNextButton.setBounds (rx, boxY, 24, boxH);
    rx -= 24;
    presetPrevButton.setBounds (rx, boxY, 24, boxH);
    rx -= 130;
    presetBox.setBounds (rx, boxY, 130, boxH);
    ...
    // --- OSC section (below MIX, bottom of right panel) ---
    oscHeaderY = row3Y + knobH + 20;
    int oscCtrlY = oscHeaderY + 16;
    oscToggleButton.setBounds (rpX, oscCtrlY, 42, 20);
    oscPortLabel.setBounds (rpX + 42 + 8 + 34, oscCtrlY, 54, 20);
